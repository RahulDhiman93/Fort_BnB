...
// GetRestrictionForRoomByDate gets the restrictions for a room filtered by date range
func (m *postgresDBRepo) GetRestrictionForRoomByDate(roomID int, start, end time.Time) ([]models.RoomsRestriction, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()
	...
	for rows.Next() {
		var rr models.RoomsRestriction
		err = rows.Scan(
			&rr.ID,
			&rr.ReservationID,
			&rr.RestrictionID,
...
// DeleteBlockByID deletes a room restrictiom
func (m *postgresDBRepo) DeleteBlockByID(id int) error {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()

	query := `delete from room_restrictions where room_id = $1`
